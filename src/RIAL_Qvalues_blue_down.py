...
class Agent():
    ...
    def selector(self, observation, done, msg):
        ...
        if random.random() < self.epsilon:
            ...
            message = random.random() if not done else 0 # random.choice(range(self.n_messages)) 
        else: 
            with torch.no_grad():
...
class Runner():
    ...
    def train(self, training_team):
        writer = SummaryWriter('src/runs/RIAL_Qvalues_blue_down')
    ...
    def save(self, episode):
        filename = f"RIAL_Qvalues_blue_down.pk"
...
def main():
    configuration_file = r'configuration_Qvalues_blue_down.yaml'
