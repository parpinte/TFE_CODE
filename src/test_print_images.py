...
class Runner():
    def __init__(self, configuration_file ):
        ...
        self.env = defense_v1.env(terrain= self.TERRAIN, max_cycles = self.MAX_CYCLES)
        self.env.reset()
        self.blue_team, self.red_team = self.teams_creation()
        self.NAME_AGENTS = self.env.agents
        self.STATE_SHAPE = int(self.env.observation_space(self.NAME_AGENTS[0])['obs'].shape[0] - 5)
    ...
    def info(self):
        ...
        state = obs['obs']
        new_state = np.concatenate((state[:4], state[9:]))
        new_obs = {'obs' : new_state, 'action_mask' : mask}
        
 
        return new_obs, r, done, i

    def demo(self,N_times, blue_epsilon, red_epsilon, render, file_name):
        ...
        for _ in range(N_times): 
            ...
            for agent in self.env.agent_iter():
                ...
                counter +=1
                if render:
                    self.env.render(file_name = filename + str(counter))
...
if __name__ == '__main__':
    configuration_file = r'parameters_launch_test_partial.yaml'
    runner = Runner(configuration_file)
    # load the neural network parameters 
    file_path_blue = os.path.abspath('nets/RIAL/ VDN_partial_observation.pk')
    ...
    render = False
    filename = 'test'
    messages_blue, messages_red, cumulative_reward_blue,cumulative_reward_red, n_cycles = runner.demo(N_times, blue_epsilon, red_epsilon, render, filename)
    ...
    # scipy.io.savemat('RIAL-10m vs RIAL-2m.mat', {'var_msg': var_msg,'cumulative_reward_blue' : cumulative_reward_blue ,'n_cycles' :n_cycles})
